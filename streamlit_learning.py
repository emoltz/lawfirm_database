...
data = pd.DataFrame(run_query("SELECT * " "from lawyers;")).rename(
    columns={
        0: "lid",
        1: "firstname",
        2: "lastname",
...
if selected == "Home":
    with header:
        ...
        number_input = st.number_input(
            "Enter a client id", min_value=0, max_value=500, value=50, step=1
        )
        ...
        selected_element = st.selectbox(
            "Pick an item from this drop-down menu", ("item1", "item2", "item3")
        )
...
if selected == "Lawyers":
    ...
    with header:
        ...
        lawyer_dropdown_col = lefthand_col.selectbox(
            "Select a lawyer you want to search for:",
            options=["Ethan", "Julie", "Sharon", "Julia", "no limit"],
            index=0,
        )
# ------------------------------------------- Cases -------------------------------------------
if selected == "Cases":
    with header:
        ...
        st.markdown(
            """<hr style="height:5px;border:none;color:#222;background-color:#333;" /> """,
...
if selected == "Research":
    ...
    with header:
        ...
        input_feature = st.text_input(
            "Type in what research document you want:", "Contract 152", key=0
        )
...
with model_training:
    ...
    sel_col.slider("What year are you searching?", min_value=2010, max_value=2020)

    # dialogue box with a default value of 'lid', remove to set default to be empty
    input_feature = sel_col.text_input(
        "Which feature should be used as input feature?", "lid"
    )
